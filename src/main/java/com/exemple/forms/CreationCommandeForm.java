...
import com.exemple.beans.Client;
import com.exemple.beans.Commande;
...
public final class CreationCommandeForm {
    ...
    private Map<String, String> listeErreurs = new HashMap<String, String>();
    ...
    public Commande creerCommande(HttpServletRequest request) {
        /* L'objet métier pour valider la création d'un client existe déjà, il est donc déconseillé de dupliquer ici
           son contenu ! À la place, il suffit de passer la requête courante à l'objet métier existant et de récupérer
           l'objet Client créé.                                                                                       */
        CreationClientForm clientForm = new CreationClientForm();
        Client client = clientForm.creerClient(request);

        /* Et très important, il ne faut pas oublier de récupérer le contenu de la map d'erreurs créée par l'objet
        métier CreationClientForm dans la map d'erreurs courante, actuellement vide.                                  */
        listeErreurs = clientForm.getListeErreurs();

        // Ensuite, il suffit de procéder normalement avec le reste des champs spécifiques à une commande.
        ...
        double valeurMontant = -1; //car soit validationMontant(montant) retourne un numero positif, soit il y a exception
        try {                     // et on passe direct au catch du coup c'est pour cela que l'on crée double valeurMontant = -1
            valeurMontant = validationMontant(montant); // pour avoir qch à mettre dans commande.setMontant(valeurMontant);
    ...
    private void setErreur(String champ, String message) {
        listeErreurs.put(champ, message);
    }
